...
def load_test_dataset(path):
    ...
    for item in data:
        uv = np.array(item["uv"], dtype=np.float64).reshape(-1)
        gt = np.array(item["target"], dtype=np.float64).reshape(-1)
...
def load_model(ckpt_path):
    ...
    stats = ckpt["stats"]
    print("ckpt train_x shape:", ckpt["train_x"].shape)
    print("ckpt train_y shape:", ckpt["train_y"].shape)

    print("ckpt y_mean:", stats["y_mean"])
    print("ckpt y_std:", stats["y_std"])

    ty = ckpt["train_y"].numpy()
    print("ckpt train_y first:", ty[0])
    print("ckpt train_y min:", ty.min(axis=0))
    print("ckpt train_y max:", ty.max(axis=0))
    ...
    train_x = ckpt["train_x"].double()
    train_y = ckpt["train_y"].double()
...
def predict(model, likelihood, x, x_mean, x_std, y_mean, y_std):
    ...
    print("x_norm checksum:", np.sum(x_norm))
    ...
    print("rvec:", y[:3])
    print("t:", y[3:6])
    print("baseline uit t:", np.linalg.norm(y[3:6]))
...
def main():
    ...
    print("X checksum:", np.sum(X))
    print("X first 10:", X[0][:10])
    ...
    # =====================================================
    # BLENDER GT volgens jouw JSON-structuur
    # =====================================================

    baseline_gt = float(blender_gt["measured_baseline_m"])

    cam_L_cm = np.array(blender_gt["camera_left_location_cm"], dtype=np.float64)
    cam_R_cm = np.array(blender_gt["camera_right_location_cm"], dtype=np.float64)

    # cm -> meter
    cam_L_m = cam_L_cm / 100.0
    cam_R_m = cam_R_cm / 100.0

    # relatieve translatie camL -> camR
    t_gt_rel = cam_R_m - cam_L_m

    # jouw JSON bevat geen rotatie, dus we nemen identity aan
    R_gt_rel = np.eye(3, dtype=np.float64)

    # optioneel, als je rvec_gt nog nodig hebt
    ...
    print("ratio:", baseline_pred / baseline_gt)
